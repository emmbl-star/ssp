...
from src.components.navigation import render_navigation, URL_BY_LABEL
...
st.markdown(
    ...
    <style>
      ...
      .cmp-navbar svg {{ height: 28px; width: auto; flex-shrink: 0; }}
      .cmp-breadcrumb {{
      ...
      .cmp-crumb:hover {{ color: #374151 !important; }}
      .cmp-crumb--active {{ color: #1C95FF !important; font-weight: 600; pointer-events: none; }}
      .cmp-sep {{ color: #9CA3AF; font-size: 14px; line-height: 1; }}
      .cmp-progress-track {{
...
# ── Fixed navbar ──────────────────────────────────────────────────────────────
_crumbs = []
for _step in _CRUMB_STEPS:
    _cls = "cmp-crumb cmp-crumb--active" if _step == _CURRENT else "cmp-crumb"
    _url = URL_BY_LABEL.get(_step, "#")
    if _step == _CURRENT:
        _crumbs.append(f'<span class="{_cls}">{_step}</span>')
    else:
        _crumbs.append(f'<a href="/{_url}" class="{_cls}">{_step}</a>')
_crumb_html = '<span class="cmp-sep">›</span>'.join(_crumbs)
...
st.markdown(
    ...
    <div class="cmp-navbar">
      ...
      <div class="cmp-breadcrumb">{_crumb_html}</div>
    </div>
    <div class="cmp-progress-track"><div class="cmp-progress-fill"></div></div>
    """,
    unsafe_allow_html=True,
)

render_navigation("Compare", show_breadcrumb=False)
...
st.markdown(
    ...
    "Compare up to 10 startups and identify which has the highest predicted "
