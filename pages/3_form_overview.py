...
st.markdown(
    ...
    <style>
      ...
      /* Wide card below fixed navbar (56 px) + progress bar (3 px) + gap */
      .block-container {{
      ...
      .fop-navbar svg {{ height: 28px; width: auto; flex-shrink: 0; }}
      /* Breadcrumb pills */
      .fop-breadcrumb {{
      ...
      /* Progress bar strip (fixed, just below navbar) */
      .fop-progress-track {{
...
_crumbs = []
for step in FLOW:
    _cls = "fop-crumb fop-crumb--active" if step == _CURRENT else "fop-crumb"
    _url = URL_BY_LABEL.get(step, "#")
    if step == _CURRENT:
        _crumbs.append(f'<span class="{_cls}">{step}</span>')
    else:
        _crumbs.append(f'<a href="/{_url}" class="{_cls}">{step}</a>')
_crumb_html = '<span class="fop-sep">›</span>'.join(_crumbs)

st.markdown(
    f"""
    <div class="fop-navbar">
      ...
      <div class="fop-breadcrumb">{_crumb_html}</div>
      <a href="/fill_form" class="fop-back-btn">← Edit</a>
    </div>
    <div class="fop-progress-track">
      <div class="fop-progress-fill"></div>
    </div>
    """,
    unsafe_allow_html=True,
)

# ── Navigation (injects GLOBAL_CSS + manages state) ───────────────────────────
render_navigation("Form Overview", show_breadcrumb=False)
