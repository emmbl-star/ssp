...
with st.form("comparison_form"):
    ...
    for i in range(num_companies):
        ...
        has_roundA = fc3.checkbox(
            "Series A",
            key=f"roundA_{i}"
        )

        has_roundB = fc4.checkbox(
            "Series B",
            key=f"roundB_{i}"
        )

        has_roundC = fc5.checkbox(
            "Series C",
            key=f"roundC_{i}"
        )

        has_roundD = fc6.checkbox(
            "Series D",
            key=f"roundD_{i}"
        )
...
if submitted:
    ...
    go.Bar(
        ...
        orientation="h",
        text=results_df["Success Probability (%)"],
        textposition="outside"
    )
)
